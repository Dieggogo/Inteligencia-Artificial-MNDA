...
def crear_parser():
    ...
    parser.add_argument(
        "--arquitectura",
        choices=["mobilenet", "resnet50", "simple"],
        default="mobilenet",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Suavizado de etiquetas para mejorar generalizacion. Por defecto: 0.05",
    )
    return parser
...
def crear_loss(tf, label_smoothing):
    return tf.keras.losses.CategoricalCrossentropy(
        label_smoothing=max(0.0, min(0.2, float(label_smoothing)))
    )


def crear_modelo_simple(tf, num_clases, learning_rate, usar_aumentacion, label_smoothing):
    ...
    model.compile(
        ...
        loss=crear_loss(tf, label_smoothing),
        metrics=["accuracy"],
    )
    return model, None
...
def crear_modelo_resnet50(
    ...
    x = tf.keras.layers.Lambda(
        ...
        name="preprocesamiento_resnet50",
    )(x)
    x = base_model(x, training=weights is None)
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    ...
    model = tf.keras.Model(inputs, outputs, name="cnn_resnet50")
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),
        loss=crear_loss(tf, label_smoothing),
        metrics=["accuracy"],
...
def crear_modelo(tf, num_clases, args):
    ...
    if args.arquitectura == "resnet50":
        return crear_modelo_resnet50(
            tf,
            num_clases=num_clases,
            learning_rate=args.learning_rate,
            usar_aumentacion=not args.sin_aumentacion,
